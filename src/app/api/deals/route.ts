...
const log = logger.child({ context: 'api:deals' })
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      log.error('Deals fetch error', { error: error instanceof Error ? error.message : String(error) })
    ...
    return apiCatchAll(error, 'deals:get')
  }
}
