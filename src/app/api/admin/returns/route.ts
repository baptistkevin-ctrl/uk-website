...
const log = logger.child({ context: 'api:admin:returns' })
...
export async function GET(request: NextRequest) {
  try {
    ...
    return apiCatchAll(error, 'admin-returns:get')
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (error) {
      log.error('Error updating return', { error: error instanceof Error ? error.message : String(error) })
      return NextResponse.json({ error: 'Failed to update return' }, { status: 500 })
    }

    return NextResponse.json(data)
  } catch (error) {
    return apiCatchAll(error, 'admin-returns:put')
  }
}
