...
export default function AdminDealsPage() {
  ...
  const getDealStatus = (deal: Deal): { label: string; color: string; icon: typeof Clock } => {
    ...
    if (now > ends) {
      return { label: 'Expired', color: 'bg-slate-100 text-slate-600', icon: AlertCircle }
    }
    if (!deal.is_active) {
      return { label: 'Paused', color: 'bg-amber-100 text-amber-700', icon: EyeOff }
    }
    return { label: 'Active', color: 'bg-emerald-100 text-emerald-700', icon: Zap }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Flash Deals</h1>
          <p className="text-slate-500 mt-1">Create and manage limited-time deals</p>
        </div>
        <button
          onClick={openAddModal}
          className="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-orange-500 to-red-500 text-white rounded-xl font-medium hover:from-orange-600 hover:to-red-600 transition-all shadow-lg shadow-orange-500/25"
        >
      ...
      {filteredDeals.length === 0 ? (
        <div className="bg-white rounded-2xl p-12 text-center border border-slate-200">
          ...
          <p className="text-slate-500 mb-6">Create your first flash deal to attract customers</p>
          <button
            onClick={openAddModal}
            className="inline-flex items-center gap-2 px-5 py-2.5 bg-orange-600 text-white rounded-xl font-medium hover:bg-orange-700 transition-colors"
          >
            <Plus className="w-5 h-5" />
            Create First Deal
          </button>
              ...
              >
                ...
                <div className="p-4">
                  ...
                  <Link
                    ...
                    target="_blank"
                    className="text-sm text-slate-500 hover:text-emerald-600 flex items-center gap-1 mb-3"
                  >
                    {deal.products?.name}
          ...
          <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-xl">
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-orange-500"
              ...
              <div>
                ...
                {getSelectedProduct() && (
                  <div className="mt-2 p-3 bg-slate-50 rounded-lg flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-slate-900">{getSelectedProduct()?.name}</p>
                      <p className="text-sm text-slate-500">
              ...
              <div>
                ...
                />
                <p className="text-xs text-slate-500 mt-1">Limit how many units can be sold at deal price</p>
              </div>
              ...
              <label className="flex items-center gap-3 p-4 bg-amber-50 rounded-xl cursor-pointer hover:bg-amber-100 transition-colors">
                ...
                <div>
                  ...
                  <p className="text-xs text-amber-700">Show prominently on the homepage</p>
                </div>
              </label>
            </div>
            ...
            <div className="flex items-center gap-3 p-6 border-t border-slate-200 bg-slate-50 rounded-b-2xl">
              ...
              <button
                onClick={handleSave}
                disabled={saving}
                className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-gradient-to-r from-orange-500 to-red-500 text-white rounded-xl font-medium hover:from-orange-600 hover:to-red-600 transition-colors disabled:opacity-50"
              >
                {saving ? (
                  <>
